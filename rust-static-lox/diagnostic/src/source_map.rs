...
#[cfg(any(test, coverage))]
pub fn with_forced_readdir_entry_error<T>(f: impl FnOnce() -> T) -> T {
  FORCE_READDIR_ENTRY_ERROR.with(|flag| {
...
impl SourceMap {
  ...
  pub(crate) fn get_files(&mut self, path: &str) -> Result<(), std::io::Error> {
